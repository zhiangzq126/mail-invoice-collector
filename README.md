...
```bash
git clone https://github.com/zhiangzq126/mail-invoice-collector.git
cd mail-invoice-collector
COLLECTOR_DIR="$PWD"
...
mkdir -p "$HOME/invoice-work"
cd "$HOME/invoice-work"

# 3. Copy the configuration templates, then edit them
cp "$COLLECTOR_DIR/config.example.yaml" config.yaml
cp "$COLLECTOR_DIR/.env.example" .env
...
python "$COLLECTOR_DIR/scripts/run.py" --config ./config.yaml folders

# 5. Preview matching messages, then run the full workflow
python "$COLLECTOR_DIR/scripts/run.py" --config ./config.yaml run --dry-run
python "$COLLECTOR_DIR/scripts/run.py" --config ./config.yaml run
```
...
This skill is a lightweight, **pure-Python, single-command, incremental** collector for routine invoice collection. [mail-invoice-pipeline](https://github.com/zhiangzq126/mail-invoice-pipeline) combines Node.js and Python in a **step-by-step workflow**, with more extensive SPA downloads and Baidu OCR fallback for bulk or difficult cases. Either skill can be used independently; this collector can reuse the pipeline's `baidu_ocr` module for OCR fallback.

The pipeline is a separate repository, not bundled here. If you enable OCR, clone it separately, configure its OCR credentials locally, and set `ocr.scripts_dir` to the absolute path of its `scripts` directory.
