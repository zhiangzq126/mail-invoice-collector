...
以下命令在 Bash 中运行（Windows 使用 Git Bash）：
...
本技能是**纯 Python、单命令、增量**的轻量收集器，适合日常一键归集。[mail-invoice-pipeline](https://github.com/zhiangzq126/mail-invoice-pipeline) 是 Node+Python 混合的**分步流水线**，SPA 平台下载与百度 OCR 兜底更强，适合批量/疑难场景。两者可独立使用；本技能的 OCR 兜底可复用 pipeline 的 `baidu_ocr` 模块。

pipeline 已是独立仓库，不包含在本项目内。启用 OCR 时请单独克隆，在本地配置其 OCR 凭据，并将 `ocr.scripts_dir` 设置为其 `scripts` 目录的绝对路径。
